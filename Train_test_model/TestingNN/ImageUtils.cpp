...
std::pair<cv::Mat, float> LoadImage(const std::string& file_name,
  ...
  if (!img.empty()) {
    ...
    return std::make_pair(img, scale); //{img, scale};
  }
  return std::make_pair(cv::Mat(), 0); // {cv::Mat(), 0};
}
...
std::tuple<cv::Mat, float> LoadImageFitSize(const std::string& file_name,
  ...
  if (!img.empty()) {
    ...
    return std::make_tuple(img, scale); // {img, scale};
  }
  return std::make_tuple(cv::Mat(), 0);//{cv::Mat(), 0};
}
